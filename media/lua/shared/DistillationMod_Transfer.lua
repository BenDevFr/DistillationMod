...
function ISTransferEssence:start()
    ...
    -- Utilise le temps configuré dans sandbox
    local transferTime = DistillationMod.GetTransferTime()
    self.action:setTime(take * transferTime)
...
function ISTransferEssence:perform()
    ...
    if self.itemTarget > itemCurrent then
        ...
        -- Remplir le conteneur cible
        self.target:setUsedDelta(self.itemTarget * self.target:getUseDelta())

        -- Calculer le nouveau delta de la source
        local sourceUses = self.source:getDrainableUsesInt()
        if sourceUses then
            local newSourceUses = sourceUses - diff

            -- CRITIQUE : Gérer le cas où la source est vide
            if newSourceUses <= 0 then
                print("[DistillationMod] Source empty, replacing with MarmiteDistillation")

                -- Créer la marmite vide
                local playerInv = self.character:getInventory()
                local marmiteVide = playerInv:AddItem("DistillationMod.MarmiteDistillation")

                if marmiteVide then
                    -- Transférer propriétés
                    marmiteVide:setCondition(self.source:getCondition())
                    marmiteVide:setFavorite(self.source:isFavorite())
                end

                -- Retirer l'essence vide
                playerInv:Remove(self.source)

                -- Sync multi
                if isClient() and marmiteVide then
                    playerInv:sendItemsInContainer()
                end
            else
                -- Mettre à jour le delta
                self.source:setUsedDelta(newSourceUses * self.source:getUseDelta())
        ...
        -- Sync conteneur cible
        if isClient() and self.target then
...
local function createTransferContextMenu(player, context, worldobjects, essenceBrute)
    ...
    -- Tooltip dynamique
    local sourceText = getText("ContextMenu_RemplirEssence") ..
    " (" .. sourceUses .. " " .. getText("ContextMenu_Units") .. ")"


    -- Création du menu principal
    local mainOption = context:addOption(sourceText, playerObj, nil)
    local subMenu = ISContextMenu:getNew(context)
    context:addSubMenu(mainOption, subMenu)

    -- Option 1 : Tous les conteneurs (si plusieurs)
    if #containers > 1 then
        subMenu:addOption(getText("ContextMenu_TousConteneurs", #containers), playerObj, onTransferToAll, essenceBrute,
            containers)
